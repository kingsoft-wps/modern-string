...
public:
    ...
    static void _refcountful_release(ELEM* _Ptr) {
        ASSERT(_Ptr != nullptr);
        ASSERT(_peek_refcount32_value(_Ptr) >= 1);
        if ((*(std::atomic<uint32_t>*)__get_refcount32_p(_Ptr)).fetch_sub(1, std::memory_order_release) == 1) {
    ...
    static constexpr uint32_t _peek_refcount32_value(ELEM* p, bool with_acquire = false) {
        return (*(std::atomic<uint32_t>*)__get_refcount32_p(p)).load(with_acquire ? std::memory_order_acquire : std::memory_order_relaxed);
    }
